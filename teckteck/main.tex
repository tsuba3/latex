...
\newlength{\actionplanMinMargin}
\setlength{\actionplanMinMargin}{3mm}

% 場所を宣言 {名前}[上の注釈]{到着時刻}{出発時刻}
\NewDocumentCommand\spot{m O{} m m}{%
    \directlua{actionplan_add_spot(\luastringN{#1}, \luastringN{#2}, \luastringN{\scriptsize#3}, \luastringN{\scriptsize#4})}
}
...
function actionplan_print_impl()
    ...
    local pathMinMargin = tex.skip['actionplanMinMargin'].width
    local currentLine = 0
    local cnt = 0
    local pathCnt = 0
    local line = {}
    ...
    local function make_spot(spot, x, y)
        ...
        local box = "\\node [draw,anchor=west] ("..name..") at ("..x.."sp, "..y.."sp) {"..spot.name.."};"
    ...
    local function writeLine(is_last)
        ...
        -- \coordinate
        tex.sprint("\\coordinate ("..S..") at (0, "..y..");")
        tex.sprint("\\coordinate ("..E..") at (0, "..y..");")
        ...
        for i=1,#line do
            ...
            if line[i].type == "path" then
                draw = draw..make_path(line[i].val)
                x = x + line[i].val.width
            end
        end
    ...
    for i = 1,#actionplan.path do
        pushSpot(actionplan.spot[i])
        pushPath(actionplan.path[i])
    end
    pushSpot(actionplan.spot[#actionplan.spot])
    writeLine(true)
